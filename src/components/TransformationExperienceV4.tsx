/**
 * TransformationExperience - v4.0
 * 
 * CHANGES from v3.4:
 * - Integrated with NarrativeController
 * - Button power-up tied to narrative progress (not arbitrary timer)
 * - Uses SankeyFlowV4 with layer-by-layer reveals
 */

import { useState, useCallback, useEffect, useRef } from 'react';
import { Share2, Eye } from 'lucide-react';
import { SankeyFlowV3 as SankeyFlowV4, FlowState } from './SankeyFlowV4';
import { BrandConfig, DEFAULT_BRAND, resolveTheme } from './branding/brandUtils';
import { UserMenu } from './ui/UserMenu';
import { NarrativePhase } from '../hooks/useNarrativeController';
...
export const TransformationExperience = ({
  ...
  // Handle narrative phase changes from SankeyFlowV4
  const handleNarrativePhaseChange = useCallback((phase: NarrativePhase) => {
    setNarrativePhase(phase);
    
    // Update power level based on narrative progress
    const phaseProgress: Record<NarrativePhase, number> = {
      'idle': 0,
      'setup-0': 10,
      'setup-1': 25,
      'setup-2': 40,
    ...
    setPowerLevel(phaseProgress[phase] || 0);
  }, []);
  
  // Handle button ready state from SankeyFlowV4
  const handleButtonReady = useCallback((ready: boolean) => {
    setIsButtonReady(ready);
    if (ready) {
      setPowerLevel(100);
    }
  }, []);
  
  // Handle narrative complete
  ...
  const handleTransform = useCallback((e?: React.MouseEvent<HTMLButtonElement>) => {
    ...
    // Reset narrative state for the new variant
  ...
  // For "after" state, button is immediately ready (uses forge animation instead)
  useEffect(() => {
    if (!isBefore) {
      setIsButtonReady(true);
      setPowerLevel(100);
    } else if (!readOnly) {
      // For "before" state, narrative controller handles button readiness
      // Only reset if not already controlled by narrative
      if (!narrativeComplete) {
        setIsButtonReady(false);
        setPowerLevel(0);
      }
    }
  }, [isBefore, readOnly, narrativeComplete]);

  // Get phase-appropriate insight text for the action zone
  const getInsightText = (): string => {
    if (!isBefore) {
      return insight || "See how this transformation unlocks value.";
    }
    
    // Phase-specific prompts for "before" state
    switch (narrativePhase) {
      case 'idle':
      case 'setup-0':
      case 'setup-1':
        return "Watch the current process unfold...";
      case 'setup-2':
      case 'setup-3':
        return "Notice where value flows...";
      case 'bleed':
        return "⚠️ See where value is leaking...";
      case 'ready':
      case 'complete':
        return "What could your process unlock?";
      default:
        return "What could your process unlock?";
    }
  };
    ...
    <div className="relative w-screen h-screen overflow-hidden bg-[#0A0A0A]">
      ...
      <div className="relative w-full h-full">
        ...
        >
          <SankeyFlowV4
            ...
            useNarrative={true}
            onNarrativePhaseChange={handleNarrativePhaseChange}
        ...
        >
          <SankeyFlowV4
            ...
            useNarrative={false}  // After state uses forge animation, not narrative
          />
        </div>
      </div>
      ...
      <div className="absolute top-6 right-6 z-30 flex items-center gap-3">
        ...
        {onPreviewModeChange && (
          ...
          >
            <Eye size={16} />
            {previewMode === 'seller' ? 'Seller View' : 'Champion View'}
          </button>
        )}
        ...
        >
          <div className="max-w-md">
            <p className="text-sm" style={{ color: 'rgba(255, 255, 255, 0.8)' }}>
              <span style={{ color: isBefore ? (narrativePhase === 'bleed' ? '#FF6B6B' : '#FFD93D') : '#4ADE80' }}>
                {isBefore ? (narrativePhase === 'bleed' ? '⚠️' : '💡') : '💡'}
              </span>
              {' '}
            ...
            style={{
              background: isBefore
                ? (() => {
                    ...
                    // Enhanced gradient during bleed phase
                    const bleedBoost = narrativePhase === 'bleed' ? 0.1 : 0;
                    return `linear-gradient(90deg, rgba(0, 212, 229, ${0.1 + power * 0.2 + bleedBoost}) 0%, rgba(0, 212, 229, ${0.15 + power * 0.25 + bleedBoost}) ${power * 100}%, rgba(255, 255, 255, 0.05) ${power * 100}%, rgba(255, 255, 255, 0.05) 100%)`;
                  })()
                : (isHovered
          ...
          >
            ...
            {isBefore && isHovered && isButtonReady && (
              <span
            ...
            {isBefore && (
              <span
                ...
                style={{
                  opacity: isHovered && isButtonReady ? 1 : 0,
            ...
            <span className="relative z-10">
              {isBefore 
                ? (isButtonReady ? 'See the Shift' : 'Loading...') 
                : 'View Current State'}
            </span>
            <svg
              width="16"
              ...
              style={{
                transform: isBefore
                  ? (isHovered && isButtonReady ? 'translateX(4px)' : 'translateX(0)')
                  : 'rotate(180deg)',
